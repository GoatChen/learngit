...
<div class="row">
	...
	<div class="col-md-12">
		<div class="x_panel">
			<div class="x_title">
				<h2>
					商品 审核列表 <i class="fa fa-user"></i><small>
						- 您可以通过搜索或者其他的筛选项对商品的信息进行审核操作。^_^</small>
				</h2>
				<div class="clearfix"></div>
			</div>
			<div class="x_content">
				<form method="post" action="${pageContext.request.contextPath }/adminuser/getgoodslist">
			    ...
			    <ul>
					<li>
						<div class="form-group">
							...
							<div class="col-md-6 col-sm-6 col-xs-12">
								<input name="queryShopName" type="text" class="form-control col-md-7 col-xs-12" value="${queryShopName }">
							</div>
						</div>
					</li>
					...
					<li>
						<div class="form-group">
							<label class="control-label col-md-3 col-sm-3 col-xs-12">商品状态</label>
							<div class="col-md-6 col-sm-6 col-xs-12">
							...
							<div class="col-md-6 col-sm-6 col-xs-12">
								<select id="queryCategoryLevel1" name="queryCategoryLevel1" class="form-control">
									...
									<c:if test="${categoryLevel1List != null }">
									   <option value="">--请选择--</option>
									   <c:forEach var="shopone" items="${categoryLevel1List }" >
									        <c:if test="${shopone.id == queryCategoryLevel1 }">selected="selected"</c:if>
									   		<option value="${shopone.id}">${shopone.categoryName}</option>
									   </c:forEach>
									</c:if>
				...
				<div class="row">
					<div class="col-sm-12">
						<table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap dataTable no-footer dtr-inline collapsed"
							...
							<tbody>
								<c:forEach var="shop" items="${shopGoodsStorage }" varStatus="status">
									<tr role="row" class="odd">
										<td tabindex="0" class="sorting_1">${shop.name}</td>
										<td>
										   </br>
                                           <img width="80" height="80" src="${pageContext.request.contextPath }/${shop.picPath }" /></br></br>
										</td>
										<td>${shop.price }</td>
										<td>${shop.number }</td>
										<td>${shop.sell_id }</td>
										<td>${shop.categoryName1 } -> ${shop.categoryName2 } -> ${shop.categoryName3 }</td>
										<td>
										    ...
										    <c:if test="${shop.status == 4 }">
										                         已上架
										    </c:if>
										    <c:if test="${shop.status == 5 }">
										                         已下架
										    </c:if>
										</td>
										<td>${shop.size }</td>
										...
										<button type="button" class="btn btn-default checkApp" 
											appinfoid="${shop.id }" status="${shop.status }" 
											data-toggle="tooltip" data-placement="top" title="" data-original-title="查看并审核商品">审核</button>
										</td>
									</tr>
								</c:forEach>
							</tbody>
