...
$("#queryCategoryLevel1").change(function(){
	...
	if(queryCategoryLevel1 != '' && queryCategoryLevel1 != null){
		$.ajax({
			type:"GET",//请求类型
			url:"../app/queryCategoryLevel2",//请求的url
...
$(".addVersion1").on("click",function(){
	var obj = $(this);
	//alert("加载三级分类失败！");
	window.location.href="../ver/appversionadd/"+obj.attr("appinfoid");
});
//修改APP版本信息
$(".modifyVersion").on("click",function(){
	...
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		if(versionid == null || versionid == ""){
			...
			window.location.href="../dev/updateVersion/"+appinfoid;
		}
	}else{
...
$(".modifyAppInfo").on("click",function(){
	...
	var status = obj.attr("status");
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		window.location.href="../app/update/"+ obj.attr("appinfoid");
	}else{
...
//上架
$(".saleup").on("click",function(){
	var obj = $(this);
	var appinfoid = obj.attr("appinfoid");
	var versionNo = obj.attr("versionNo");
	if(versionNo==null || versionNo==""){
	   alert("该APP应用无版本信息，请先增加版本信息！");
	}else if(confirm("是否对【"+obj.attr("appsoftwarename")+"】进行【上架】操作?")){
	$.ajax({
		type:"GET",
		url:"../app/saleup",
		...
		success:function(data){
			...
			}else if(data.result == "failed"){//上架失败
...
//下架
$(".saledown").on("click",function(){
	var obj = $(this);
	var appinfoid = obj.attr("appinfoid");
	if(confirm("你确定要下架APP应用【"+obj.attr("appsoftwarename")+"】吗？")){
		$.ajax({
			type:"GET",
			url:"../app/saledown",
			data:{id:obj.attr("appinfoid")},
...
$(".viewApp").on("click",function(){
	var obj = $(this);
	window.location.href="../app/appinfoView/"+ obj.attr("appinfoid");
});
